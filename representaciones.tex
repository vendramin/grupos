...
Si $G$ es un grupo finito, un morfismo de grupos $\rho\colon G\to\GL(V)$, $g\mapsto\rho_g$, donde $V$ es un espacio
...
Si el espacio vectorial $V$ tiene dimensión $n$, al fijar una base 
para $V$ podemos considerar $\rho\colon G\to\GL(V)\simeq\GL_n(\C)$. Observemos que 
toda representación de un grupo finito será de dimensión finita. 
...
\]
para todo $g\in G$. Notación: $\phi\simeq\psi$. Observemos que $\phi\simeq\psi$ si y sólo si $V$
...
Traducimos ahora la noción de submódulo al contexto de la teoría de representaciones. 
Sea $\phi\colon V\to\GL(V)$ una representación. Un subespacio $W\subseteq V$ se dice
