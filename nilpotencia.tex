...
\begin{proof}
	...
	Veamos primero que $\zeta_{i+1}(G)$ es un subgrupo normal de $G$. 
	...
	subgrupos de $G$. 
	Además $xgx^{-1}\in\zeta_{i+1}(G)$ pues 
	\[
...
\begin{proof}
	...
	que el resultado es válido para algún $i\geq0$. Sea 
	$\pi_i\colon G\to
	G/G_i$ es el morfismo canónico.
	Entonces 
	\[
	...
	$\pi_i([G,G_{i-1}])=[\pi_i(G),\pi_i(G_{i-1})]=\{1\}$ y luego $[G,G_{i-1}]\subseteq
	G_i$.
\end{proof}
