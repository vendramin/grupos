...
\begin{proof}
	...
	identidad de Hall--Witt~\ref{exercise:HallWitt}, se concluye que 
	$[z,x^{-1},y]=1$.
\end{proof}

\begin{lemma}
	\label{lemma:3subgrupos_general}
	Sea $N$ un subgrupo normal de un grupo $G$ y sean $X,Y,Z\subseteq N$
	subgrupos. Si $[X,Y,Z]\subseteq N$ y $[Y,Z,X]\subseteq N$. Entonces
	$[Z,X,Y]\subseteq N$.
\end{lemma}
...
\begin{proof}
	...
	tres subgrupos~\ref{lemma:3subgrupos}, $[\pi(Z),\pi(X),\pi(Y)]=1$, es decir
	$[Z,X,Y]\subseteq N$.
\end{proof}
...
\begin{proof}
	...
	\[
	[\gamma_i(G),\gamma_{j+1}(G)]=[\gamma_{j+1}(G),\gamma_i(G)]=[\gamma_j(G),G,\gamma_i(G)]\subseteq \gamma_{i+j+1}(G)
	\]
...
\begin{proof}
	...
	todo $i\geq1$. La segunda afirmación se deduce del
	ejercicio~\ref{exercise:gamma}: si existe $c$ tal que $\gamma_{c+1}(G)=1$
	entonces $\gamma_{c+1}(H)=f(\gamma_{c+1}(G))=f(1)=1$. 
\end{proof}
...
\begin{proof}
	...
	$G$ es un $p$-grupo, $Z(G)\ne1$. Esto implica que $G/Z(G)$ es un $p$-grupo
...
\begin{proof}
	...
	$1=\gamma_{c+1}(G)\subseteq H$ y $\gamma_1(G)\not\subseteq H$, podemos
	tomar el mínimo $k$ tal que $\gamma_k(G)\subseteq H$. Como
	\[
...
\begin{proof}
	...
	\]
	pues $H_{k-2}$ es normal en $H_{k-1}$. Al repetir este argumento se
	concluye que $g^{(G:H)}\in H$. 
\end{proof}
...
\begin{proof}
	...
	\begin{align*}
		[h,k]&=[h,x^m][x^m,[h,d]][h,d]\\
			&=[x^n,[c,x^m]][c,x^m][x^m,[h,d]][h,d]\in\gamma_3(G).\\
	\end{align*}
%	Como
...
\begin{proof}
	...
	\[
		[x,y]^p=[x^p,y]=[1,y]=1.
	\]
	Luego, al usar el lema~\ref{lemma:Hall}, se concluye qye
